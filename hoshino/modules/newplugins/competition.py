...
sv = Service('pcr-competition',  bundle='pcr娱乐', help_='[@bot赛跑模拟]模拟来一局赛🐎')
...
@sv.on_fullmatch(('模拟赛跑','赛跑模拟','模拟赛马','模拟赛🐴'), only_to_me=True)
async def pcr_comp(bot, ev: CQEvent):
    ...
    if not lmt.check(uid):
        ...
        return
    if g_result != []:
        await bot.send(ev, '上一场比赛尚未结束，请等待\n※管理员发送“@bot赛跑重置”可重置比赛', at_sender=True)
        return
    lmt.increase(uid)
...
async def _select_(bot, ev):
    ...
    if uid != g_uid and g_result == []:
        await bot.finish(ev, f'上一场比赛已经结束，您可以用“@bot赛跑模拟”发起新的比赛', at_sender=True)
    if uid == g_uid:
        await select_player(bot, ev)
    else:
        await bot.finish(ev, f'出现错误，请联系维护组嘤嘤嘤')

@sv.on_fullmatch(('赛跑重置'))#, only_to_me=True)
async def reset_comp(bot, ev):
    global g_result, g_uid
    if not priv.check_priv(ev, priv.NORMAL):
        await bot.finish(ev, '只有群管理才能重置', at_sender=True)
    g_result = []
    g_uid = 0
    await bot.send(ev, f'比赛已经重置！', at_sender=True)
