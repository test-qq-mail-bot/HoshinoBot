...
bot = get_bot()
fd = "./hoshino/modules/newplugins/"  # 存档的绝对路径前缀，记得改成你自己的
with open(fd + 'rs-data.json') as f:
    data = json.load(f)
with open(fd + 'rs-player.json') as f:
    player = json.load(f)
...
async def spin(session: CommandSession):
    if session.ctx['message_type'] == 'private':
        ...
        if data['curnum'] == 0:
            ...
            if bullet == '6':
                ...
                if ans == 'y':
                    ...
                    data['next'] = 0
                    await session.send("装填完毕。继续发送“禁言转盘”来继续游戏")
                else:
                    session.finish("好吧.")
            elif '0' < bullet < '6':
        ...
        save(data, fd + 'rs-data.json')
        save(player, fd + 'rs-player.json')
