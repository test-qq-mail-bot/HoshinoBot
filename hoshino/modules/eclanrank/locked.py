...
def default_rank(group_id):
    group = db.get(group_id, [])
    if not group:
        return '还没有绑定公会呢 快用 会战锁定#公会名 来进行绑定'
    res = []
    for value in group:
...
async def check_rank_state():
    group_list = groupby(list(*db.values()), lambda x: x['clan_name'])
...
scheduler.add_job(check_rank_state, 'cron', hour=f"*/{config['rules']['lock_clan_cron_time']}")
